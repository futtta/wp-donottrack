...
function sanitizer(tS) {
	...
	try {tS=tS.match(re)[1].toString();} catch(e) {return tS;}

	if (mode==='blacklist') {
		// todo: replace "for in" with normal for loop (avoid probs such as those with prototype
        	for (x in list['black']) {
			if (typeof list['black'][x]==="string") {
...
// todo: can't we have one single check (in the javascript in the calling page) to see if we need to filter and then set "doFilter=true"?
if ((dnt_config.ifdnt!=="1")||(navigator.doNotTrack==="yes")||(navigator.msDoNotTrack==="1")||(navigator.doNotTrack==="1")||(document.cookie.indexOf("dont_track_me=1")!==-1)||(document.cookie.indexOf("civicAllowCookies=no")!==-1)) {
	...
	// todo: do I really want to keep these?
