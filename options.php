...
function wp_donottrack_add_admin_menu() {
	add_options_page( 'WP DoNotTrack', 'Options for WP DoNotTrack', 'manage_options', 'wp-donottrack', 'wp_donottrack_options_page_render' );
}
...
function wp_donottrack_settings_init() {
	...
	add_settings_field(
		'wp_donottrack_blacklist',
		__( 'Blacklist', 'wp-donottrack' ),
		'wp_donottrack_blacklist_render',
		'wp-donottrack',
		'wp_donottrack_list_section' );

	add_settings_field(
		'wp_donottrack_whitelist',
		__( 'Whitelist', 'wp-donottrack' ),
...
function wp_donottrack_scope_render() {
	...
	?>
	<fieldset>
		<label title="<?php __( 'Only stop tracking for people with the DoNotTrack browser setting (does not work in Chrome) or based on the presence of a cookie.', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[scope]' <?php checked( $options['scope'], 1 ); ?> value='1'><?php __( "Stop tracking for people who have their <a href=\"http://wordpress.org/extend/plugins/wp-donottrack/faq/\">browser configured to do so or based on the presence of a cookie</a>.", "wp-donottrack" ); ?></label></br>
		<label title="<?php __( 'Privacy for all!', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[scope]' <?php checked( $options['scope'], 0 ); ?> value='0'><?php __( "Disable tracking for all my visitors!", "wp-donottrack" ); ?></label><br />
	</fieldset>
	<?php
}
...
function wp_donottrack_level_render() {
	...
	?>
	<fieldset>
		<label title="<?php __( 'Normal', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[level]' <?php checked( $options['level'], 0 ); ?> value='0'><?php __( "Normal (least invasive)", "wp-donottrack" ); ?></label></br>
		<label title="<?php __( 'Forced', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[level]' <?php checked( $options['level'], 1 ); ?> value='1'><?php __( "Forced (default)", "wp-donottrack" ); ?></label><br />
		<label class="wp_donottracksuperclean" title="<?php __( 'SuperClean', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[level]' <?php checked( $options['level'], 2 ); if($options['scope']===1) echo "style=\"display:none\""; ?> value='2' ><?php __( "SuperClean (most invasive)", "wp-donottrack" ); ?></label><br />
	</fieldset>
	<?php
}
...
function wp_donottrack_listmode_render() {
	...
	?>
	<fieldset>
		<label title="<?php __( 'Blacklist', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[listmode]' <?php checked( $options['listmode'], 0 ); ?> value='0'><?php __( "Blacklist", "wp-donottrack" ); ?></label></br>
		<label title="<?php __( 'Whitelist', 'wp-donottrack' ); ?>"><input type='radio' name='wp_donottrack_settings[listmode]' <?php checked( $options['listmode'], 1 ); ?> value='1'><?php __( "Whitelist", "wp-donottrack" ); ?></label><br />
	</fieldset>
	<?php
}

function wp_donottrack_blacklist_render() {
	$options = wp_donottrack_get_option();
	?>
	<div id="blacklistdiv" <?php if($options['listmode']!==0) echo "style=\"display:none\""; ?>>
		<input type="text" name="wp_donottrack_settings[blacklist]" value="<?php $options['blacklist'] ?>" size="80" class="regular-text code" /><br />
		<span class="description"><?php __( "Comma-seperated list of the domains you want to exclude from being added to your blog", "wp-donottrack" ) ?></span>
	</div>
	<?php
}

function wp_donottrack_whitelist_render() {
	$options = wp_donottrack_get_option();
	?>
	<div id="whitelistdiv" <?php if($options['listmode']!==1) echo "style=\"display:none\""; ?>>
		<input type="text" name="wp_donottrack_settings[whitelist]" value="<?php $options['whitelist'] ?>" size="80" class="regular-text code" /><br />
		<span class="description"><?php __( "Comma-seperated list of the domains you want to allow to be added to your blog (your blog will be auto-whitelisted)", "wp-donottrack" ) ?></span>
	</div>
	<?php
}
...
function wp_donottrack_thirdparty_addthis_render() {
	$options = wp_donottrack_get_option();
	?>
	<label title="<?php __( 'Use AddThis plugin without cookies', 'wp-donottrack' ) ?>"><input type='checkbox' name='wp_donottrack_settings[thirdparty][addthis]' <?php checked( $options['thirdparty']['addthis'], 1 ); ?> value='1'><?php __( "Instruct the <a href=\"https://wordpress.org/plugins/addthis/\" target=\"_blank\">AddThis plugin</a> to not use cookies.", "wp-donottrack" ) ?></label>
	<?php
}
...
function wp_donottrack_thirdparty_addtoany_render() {
	$options = wp_donottrack_get_option();
	?>
	<label title="<?php __( 'Instruct Add-To-Any plugin to not use 3rd party cookies', 'wp-donottrack' ) ?>"><input type='checkbox' name='wp_donottrack_settings[thirdparty][add-to-any]' <?php checked( $options['thirdparty']['add-to-any'], 1 ); ?> value='1'><?php __( "Tell the <a href=\"https://wordpress.org/plugins/add-to-any/\" target=\"_blank\">Add-To-Any plugin</a> to disable 3rd party cookies.", "wp-donottrack" ) ?></label>
	<?php
}
